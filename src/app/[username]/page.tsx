...
export default function ProfilePage() {
  ...
  async function getPosts(username: string, user: User, skip?: number) {
    skip = skip || 0
    const response = await axios.get(
      `https://incognitosocial.vercel.app/api/users/${username}/posts?skip=${skip}`,
    )
    const postsArray = response.data
    postsArray.map((post: Post): void => {
      post.author = user
    })
  ...
  return (
    <>
      ...
      <main>
        ...
        <ProfileNav
          section={section}
          handlePostSection={handlePostSection}
          handleRepostSection={handleRepostSection}
        ...
        {section === 'repost' &&
          (posts ? (
            posts.map(
              (post) =>
                post.type === 'repost' && (
                  <RepostComponent key={post._id} post={post} />
                ),
            )
          ) : (
            <p>Sem Posts</p>
