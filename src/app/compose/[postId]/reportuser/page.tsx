...
import useReportUser from '@/app/components/customHooks/useReportUser'
...
export default function ComposeRepost() {
  ...
  const HandleReport = async () => {
    ...
    const response = await useReportUser(username, text, jwt)
    if (response) {
      router.push('/home')
    }
    buttonRef.current?.removeAttribute('disabled')
  ...
  return (
    <main className="w-screen h-screen flex justify-center items-center">
      ...
      <article className="rounded-lg border p-4 max-w-[440px]">
        <h1 className="text-2xl font-bold ml-6 my-2">Report User</h1>
        {session.data ? (
          <>
